...
export const AdminCloudVMsPage: React.FC = () => {
  ...
  const fetchCloudVMs = async () => {
    try {
      const response = await axios.post(`${process.env.url}:3000/api/v1/lab_ms/getLabsConfigured`
, {
        admin_id: admin.id,
      });
  ...
  const fetchDatacenterVMs = async () => {
    try {
      const dcResponse = await axios.post(`${process.env.url}:3000/api/v1/lab_ms/getDatacenterLabOnAdminId`
, {
        adminId: admin?.id,
      });
