...
export const AIRecommendations: React.FC<AIRecommendationsProps> = ({ config, onConfirm }) => {
  ...
  useEffect(() => {
    const fetchRecommendations = async () => {
      try {
        const response = await axios.post(`${process.env.url}:3000/api/v1/lab_ms/getInstances`
, {
  ...
  const deleteLabData = async (lab_id: string) => {
    try {
      await axios.delete(`${process.env.url}:3000/api/v1/aws_ms/deleteLab`
, { data: { lab_id } });
    } catch (err) {
  ...
  const handleData = async (instanceName: string, region: string) => {
    ...
    try {
      const response = await axios.post(`${process.env.url}:3000/api/v1/lab_ms/labconfig`
, {
        data,
        user: user_cred,
      });
      ...
      const pythonRes = await axios.post(`${process.env.url}:3000/api/v1/aws_ms/python`
, {
        cloudPlatform: config.cloudProvider,
        lab_id,
      });

      const tfRes = await axios.post(`${process.env.url}:3000/api/v1/aws_ms/pythontf`
, { lab_id });
      ...
      const instancedetails = await axios.post(`${process.env.url}:3000/api/v1/lab_ms/awsCreateInstanceDetails`
, {
        lab_id,
      });

      await axios.post(`${process.env.url}:3000/api/v1/aws_ms/decryptPassword`
, {
        lab_id,
