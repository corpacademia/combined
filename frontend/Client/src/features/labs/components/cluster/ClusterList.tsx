...
export const ClusterList: React.FC = () => {
  ...
  const fetchClusters = async () => {
    try {
      setLoading(true);
      const userProfile = await axios.get(`${process.env.url}:3000/api/v1/user_ms/user_profile`
);
      ...
      if(userProfile.data.user.role === 'superadmin'){
        response = await axios.post(`${process.env.url}:3000/api/v1/vmcluster_ms/getClusterLabs`
,{
        userId:userProfile.data.user.id
      });
      }
      else if(userProfile.data.user.role === 'orgadmin'){
         response = await axios.post(`${process.env.url}:3000/api/v1/vmcluster_ms/getOrglabs`
,{
