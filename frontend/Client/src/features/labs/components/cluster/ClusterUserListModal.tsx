...
export const ClusterUserListModal: React.FC<ClusterUserListModalProps> = ({ 
 ...
 const groupedUsers = users.reduce((acc, user) => {
  ...
  const handleToggleDisable = async (vmData: VM) => {
    ...
    try {
      const response = await axios.post(`${process.env.url}:3000/api/v1/vmcluster_ms/updateClusterVmCredentials`
, {
        id: vmData.id,
...
, {
  ...
  const handleSaveVM = async (vmData: VM) => {
    try {
      const response = await axios.post(`${process.env.url}:3000/api/v1/vmcluster_ms/editClusterVmCredentials`
, {
        id: vmData.id,
