...
export const AssignLabModal: React.FC<AssignLabModalProps> = ({
  ...
  const [admin,setAdmin] = useState({});
   useEffect(() => {
    const getUserDetails = async () => {
      const response = await axios.get(`${process.env.url}:3000/api/v1/aws_ms/user_profile`
);
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        const data = await axios.post(`${process.env.url}:3000/api/v1/lab_ms/getLabsConfigured`
, {
          admin_id: admin.id
        });
      ...
      if (response.data.success) {
        const options = {
          ...
          handler: async (response: any) => {
            try {
              const verifyResponse = await axios.post(`${process.env.url}:3000/api/v1/verify-payment`
, {
  ...
  const handleAssign = async () => {
    ...
    try {
      const assign = await axios.post(`${process.env.url}:3000/api/v1/lab_ms/assignlab`
, {
        lab: lab,
        duration: duration,
        userId: userId,
