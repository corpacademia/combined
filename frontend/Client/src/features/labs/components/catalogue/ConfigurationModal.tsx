...
export const ConfigurationModal: React.FC<ConfigurationModalProps> = ({
  ...
  const handleConfigurations = async () => {
    ...
    try {
      ...
      const updateConfig = await axios.post(`${process.env.url}:3000/api/v1/lab_ms/updateConfigOfLabs`
, {
        lab_id: lab.lab_id,
        admin_id: user.id,
...
,{lab_id:lab.lab_id})
        ...
        if(ami.data.success){
          try {
            const golden_instance = await axios.post(`${process.env.url}:3000/api/v1/aws_ms/goldenToInstance`
,{
