...
export const AssignUsersModal: React.FC<AssignUsersModalProps> = ({
  ...
  React.useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await axios.post(`${process.env.url}:3000/api/v1/user_ms/getOrganizationUsers`
, {
          admin_id: admin.id
        });
  ...
  const handlePayment = async () => {
    ...
    try {
      const response = await axios.post(`${process.env.url}:3000/api/v1/initiate-payment`
, {
        lab_id: lab?.lab_id,
        user_id: admin.id,
  ...
  const handleAssignUsers = async () => {
    ...
    if(type ==='cloudslice'){
      const response = await axios.post(`${process.env.url}:3000/api/v1/cloud_slice_ms/assignCloudSlice`
, {
        lab: lab?.labid,
      ...
      const response = await axios.post(`${process.env.url}:3000/api/v1/lab_ms/assignSingleVmDatacenterLabToUser`
, {
        labId:lab?.lab_id,
      ...
      const response = await axios.post(`${process.env.url}:3000/api/v1/vmcluster_ms/assignCluster`
, {
        labId: lab?.labid,
        userId: selectedUsers,
        assignedBy: admin.id,
     ...
     const response = await axios.post(`${process.env.url}:3000/api/v1/lab_ms/assignlab`
, {
        lab: lab?.lab_id,
        userId: selectedUsers,
        assign_admin_id: admin.id
