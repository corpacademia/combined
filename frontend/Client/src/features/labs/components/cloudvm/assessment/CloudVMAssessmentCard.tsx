  ...
  useEffect(() => {
    const fetchOrg = async () => {
      if (assessment.config_details?.organizationId) {
        const organizationDetails = await axios.post(`${process.env.url}:3000/api/v1/organization_ms/getOrgDetails`
, {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const user_cred = await axios.get(`${process.env.url}:3000/api/v1/user_ms/user_profile`
);
