      ...
      try {
        ...
        if(user.role ==='superadmin'){
          const [standardResult, cloudResult] = await Promise.allSettled([
            axios.post(`${process.env.url}:3000/api/v1/lab_ms/getLabsConfigured`
, {
              admin_id: user.id,
            }),
  ...
  const handleAssign = async () => {
    ...
    try {
      ...
      if (selectedLabDetails.type === 'cloudslice') {
        ...
        res = await axios.post(`${process.env.url}:3000/api/v1/cloud_slice_ms/assignCloudSlice`
, {
        ...
        res = await axios.post(`${process.env.url}:3000/api/v1/lab_ms/assignlab`
, {
