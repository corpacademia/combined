...
export const UsersPage: React.FC = () => {
  ...
  useEffect(() => {
    const getUsers = async () => {
      try {
        const response = await axios.get(`${process.env.url}:3000/api/v1/user_ms/allUsers`
);
...
, {
  ...
  const handleBulkUpload = async (uploadedUsers: any[]) => {
    try {
      const result = await axios.post(`${process.env.url}:3000/api/v1/user_ms/bulkUploadUsers`
, {
        users: uploadedUsers,
        admin: admin
      });
