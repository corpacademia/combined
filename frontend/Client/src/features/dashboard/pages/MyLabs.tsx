...
const DeleteModal: React.FC<DeleteModalProps> = ({ isOpen, onClose, labId, labTitle, userId }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      try {
        instance_details = await axios.post(`${process.env.url}:3000/api/v1/lab_ms/awsInstanceOfUsers`
, {
          lab_id: labId,
          user_id: userId,
        });
      ...
      let ami;
      try {
        ami = await axios.post(`${process.env.url}:3000/api/v1/lab_ms/amiinformation`
, { lab_id: labId });
      } catch (error) {
      ...
      let response;
      try {
        response = await axios.post(`${process.env.url}:3000/api/v1/aws_ms/deletevm`
, {
          id: labId,
 ...
 useEffect(() => {
  const fetchData = async () => {
    ...
    try {
      ...
      try {
        const res = await axios.get(`${process.env.url}:3000/api/v1/lab_ms/getCatalogues`
);
        cats = res.data.data;
      } catch (err) {
      ...
      try {
        const res = await axios.post(`${process.env.url}:3000/api/v1/lab_ms/getAssignedLabs`
, { userId: user.id });
        labss = res.data.data;
      ...
      try {
        const res = await axios.get(`${process.env.url}:3000/api/v1/lab_ms/getSoftwareDetails`
);
        ...
        if (res.data.success) {
          const vmDetails = await Promise.all(
            res.data.data.map(async (assignment: any) => {
              try {
                const vmRes = await axios.post(`${process.env.url}:3000/api/v1/lab_ms/getSingleVmDatacenterLabOnId`
, {
                ...
                if (vmRes.data.success) {
                  const credsRes = await axios.post(
                    `${process.env.url}:3000/api/v1/lab_ms/getUserAssignedSingleVMDatacenterCredsToUser`
,
      ...
      try {
        ...
        if (res.data.success) {
          const clusterDetails = await Promise.all(
            res.data.data.map(async (assignment: any) => {
              try {
                const clusterRes = await axios.post(`${process.env.url}:3000/api/v1/vmcluster_ms/getClusterLabOnId`
, {
                  labId: assignment.labid,
                });
                ...
                if (clusterRes.data.success) {
                  const usersRes = await axios.post(
                    `${process.env.url}:3000/api/v1/vmcluster_ms/getUserAssignedClusterCredsToUser`
,
                    { labId: assignment.labid, userId: user.id }
                  );
  ...
  const checkLabStatus = async (labId: string) => {
    try {
      const response = await axios.post(`${process.env.url}:3000/api/v1/aws_ms/checkLabStatus`
, {
        lab_id: labId,
        user_id: user.id
      });
  ...
  const handleLaunchLab = async (lab) => {
    ...
    try {
      const [ami, labConfig] = await Promise.all([
        axios.post(`${process.env.url}:3000/api/v1/lab_ms/amiinformation`
, { lab_id: lab.lab_id }),
        axios.post(`${process.env.url}:3000/api/v1/lab_ms/getAssignLabOnId`
, { labId: lab.lab_id ,userId:user.id}),
      ...
      const response = await axios.post(`${process.env.url}:3000/api/v1/aws_ms/launchInstance`
, {
        name: user.name,
  ...
  const handleStartStopLab = async (lab) => {
    ...
    const cloudinstanceDetails = await axios.post(`${process.env.url}:3000/api/v1/aws_ms/getAssignedInstance`
, {
      user_id: user.id,
      lab_id: lab.lab_id,
    })
    ...
    try {
      ...
      console.log(isStop)
      if (isStop) {
        const stop =await axios.post(`${process.env.url}:3000/api/v1/aws_ms/stopInstance`
, {
          instance_id: instanceId
        });
        if(stop.data.success){
          await axios.post(`${process.env.url}:3000/api/v1/lab_ms/updateawsInstanceOfUsers`
,{
            lab_id:lab.lab_id,
            user_id:user.id,
            state:false,
      ...
      const checkInstanceAlreadyStarted = await axios.post(`${process.env.url}:3000/api/v1/lab_ms/checkisstarted`
,{
        type:'user',
      ...
      if(checkInstanceAlreadyStarted.data.isStarted === false){
          ...
          console.log('stop')
          const response = await axios.post(`${process.env.url}:3000/api/v1/aws_ms/runSoftwareOrStop`
, {
            os_name: lab.os,
...
,{
        ...
        const restart = await axios.post(`${process.env.url}:3000/api/v1/aws_ms/restart_instance`
, {
          instance_id: cloudinstanceDetails?.data.data.instance_id,
          user_type:'user'
        });
...
,{
  ...
  const handleDeleteCloudSlice = async (labId: string, labStatus: any) => {
    ...
    try {
      let response;
      const deleteIam = await axios.post(`${process.env.url}:3000/api/v1/aws_ms/deleteIamAccount`
, {
        userName: acountDetails.username
      })
      if (deleteIam.data.success) {
        response = await axios.post(`${process.env.url}:3000/api/v1/cloud_slice_ms/deleteUserCloudSlice`
, {
          userId: user.id,
          labId: labId
        })
  ...
  const handleDeleteDatacenterVM = async (labId: string) => {
    try {
      const response = await axios.post(`${process.env.url}:3000/api/v1/lab_ms/deleteSingleVmDatacenterUserAssignment`
, {
        labId: labId,
        userId: user.id
      });
